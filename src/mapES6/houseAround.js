...
class HouseAround {
    ...
    _renderCenterMarker (point) {
        ...
        if (self._setting.centerMarkerClick) {
            _centerMarker.addEventListener('click', () => {
                // 绑定统计事件
                if (typeof self._setting.trackEvent == 'function') {
    ...
    _renderCategoryComplexOverlay (data) {
        ...
        let _opt = {
            ...
            '_eventCallBack': function () {
                // let me = this;
                // EventWrapper.addDomListener(me._div, "touchend",  function () {
    ...
    _triggerClick (triggerHandler) {
        let self = this;
        $(self._setting.categoryElement).find(self._setting.categoryLoop).each((i, e) => {
    ...
    _bindTrafficHandler () {
        let self = this;
        //交通 tab 切换
        $(self._setting.trafficTab).click(() => {
            // 绑定统计事件
            if (typeof self._setting.trackEvent == 'function') {
    ...
    _renderTrafficMapInfo (traffic) {
        ...
        for (let i in traffic) {
            ...
            let _opt = {
                ...
                '_eventCallBack': function() { //基于BMap.lib 绑定自定义覆盖物点击事件
                    let me = this;
                    EventWrapper.addDomListener(me._div, "touchend", function () {
                        // 绑定统计事件
                        if (typeof self._setting.trackEvent == 'function') {
